...
class FaceMeshDetector():
    ...
    def __init__(self, staticMode=False, maxFaces=2, refinelm=False, minDetectionCon=0.5, minTrackCon=0.5):
        ...
        self.faceMesh = self.mpFaceMesh.FaceMesh(self.staticMode, self.maxFaces, refinelm,
                                                self.minDetectionCon, self.minTrackCon)
        self.drawSpec = self.mpDraw.DrawingSpec(color=(0,128,0), thickness=1, circle_radius=1)
        self.drawSpecLm = self.mpDraw.DrawingSpec(color=(0,128,0), thickness=1, circle_radius=2)
            ...
            for faceLms in self.results.multi_face_landmarks:
                if draw:
                    ...
                    self.mpDraw.draw_landmarks(img, faceLms, self.mpFaceMesh.FACEMESH_CONTOURS, \
                                        self.drawSpecLm, self.drawSpecLm)
